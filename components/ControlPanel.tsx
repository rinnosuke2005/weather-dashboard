...
export default function ControlPanel({
  ...
  // 見出しのスタイル（これは共通化しておくとスッキリします）
  const labelClass = "text-xs font-bold text-gray-500 uppercase tracking-widest text-center mb-2 drop-shadow-sm";

  return (
    // ★ 親枠：すりガラス極み（bg-white/30 + backdrop-blur-2xl + rounded-3xl）
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full bg-white/30 backdrop-blur-2xl p-6 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] border border-white/50">
      ...
      <div className="flex flex-col flex-1">
        ...
        {/* ★ selectClassをなくし、直接クラスを記述。
            appearance-none と カスタム矢印(bg-[url...]) は残して、ダサい▼を消します */}
        <select
          id="city-select"
          value={city}
        ...
        >
          ...
          <option value="tokyo" className="text-gray-600 font-medium">東京 (Tokyo)</option>
          <option value="osaka" className="text-gray-600 font-medium">大阪 (Osaka)</option>
          <option value="nagoya" className="text-gray-600 font-medium">名古屋 (Nagoya)</option>
          <option value="fukuoka" className="text-gray-600 font-medium">福岡 (Fukuoka)</option>
          <option value="sapporo" className="text-gray-600 font-medium">札幌 (Sapporo)</option>
          <option value="naha" className="text-gray-600 font-medium">那覇 (Naha)</option>
        </select>
      </div>
      ...
      <div className="flex flex-col flex-1">
        ...
        <select
          ...
          className="w-full h-11 pl-4 pr-10 text-gray-700 font-bold tracking-wide bg-white/40 backdrop-blur-md border border-white/60 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400/50 cursor-pointer hover:bg-white/50 transition-all appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%236b7280%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[position:right_12px_center] bg-[length:16px_16px]"
        >
          <option value="temperature_2m" className="text-gray-600 font-medium">気温</option>
          <option value="apparent_temperature" className="text-gray-600 font-medium">体感温度</option>
          <option value="precipitation" className="text-gray-600 font-medium">降水量</option>
          <option value="wind_speed_10m" className="text-gray-600 font-medium">風速</option>
        </select>
      </div>
